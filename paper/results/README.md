# Experiment run 2026-08-06T10:55:02+00:00

**These numbers are NOT reportable.**

- commit `f29680f7024c` **(working tree dirty)**
...
- acoustic (ECAPA): scored 2205 of 2205 trials (100.0%)
- knowledge (answer matcher): scored 0 of 2205 trials (0.0%)
...
2. 7 speakers have one session, so no cross-session trial exists for them and §5.3 cannot be computed on this corpus
3. the knowledge (answer matcher) branch scored no trials at all (knowledge (answer matcher): 0/2205 trials scored (0.0%); unmeasured: claimed speaker never answered this prompt at enrolment x2205), so every fusion row that names it was in fact fused without it
